<?php
    function debit($request)
    {        
        $user = getUser($request->data->user_id, $request->data->user_game_token, $request->data->currency);
        
		if ($user->amount < $request->data->debit_amount)
        {
            throw new Exception("Not enought amount");
        }
        
        $amount = userUpdateAmount($request->data->user_id, $request->data->debit_amount, $request->data->currency);
        userUpdateGameTokenDate($request->data->user_id);
    ...
    function userUpdateAmount($user_id, $debit_amount, $currency)
    {
        mysql_query("update amount set amount = amount - $debit_amount where user = $user_id and currency = '$currency'");
    ...
    function userUpdateGameTokenDate($user_id)
    {
        mysql_query("update users set game_token_date = now() where id = $user_id");

        if (mysql_error())
        {
            throw new Exception("Update game token date error");                
        }      
    }    
?>
